  ...
  return (
    <div className="relative w-full h-[60vh] flex flex-col items-center justify-center overflow-hidden transition-all duration-700 ease-in-out">
      ...
      <div className="relative z-10 text-center px-6 max-w-3xl">
        <div className="inline-block mb-4 px-3 py-1 bg-white/50 backdrop-blur-md rounded-full border border-white/40 shadow-sm">
          <span className="text-xs font-semibold text-blue-600 tracking-wide uppercase">New Discovery Engine 2.0</span>
        </div>
        ...
        <div className="hidden md:flex justify-center gap-6 mt-12">
           <StatPill label="Papers" value="10M+" color="bg-blue-100 text-blue-700" />
           <StatPill label="Books" value="50M+" color="bg-emerald-100 text-emerald-700" />
           <StatPill label="Topics" value="200K+" color="bg-purple-100 text-purple-700" />
        </div>
      </div>
    </div>
  );
...
const StatPill: React.FC<{ label: string; value: string; color: string }> = ({ label, value, color }) => (
  <div className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl backdrop-blur-md bg-white/60 border border-white/50 shadow-sm transition hover:scale-105`}>
